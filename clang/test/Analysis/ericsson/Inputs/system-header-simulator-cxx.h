...
namespace std {
// these declarations needed in AllocWithState
template <typename A, typename B, class BINPRED, class ALLOCATOR>
class map {
	 ALLOCATOR allocator;;
};
...
template <typename Iterator> struct __vector_reverse_iterator {};
...
template <typename T, typename Ptr, typename Ref> struct __list_iterator {
  ...
  __list_iterator<T, Ptr, Ref> operator=(const iterator &rhs) {
    item = rhs.item;
    return *this;
  }
...
template <typename T, typename Ptr, typename Ref> struct __fwdl_iterator {
  ...
  __fwdl_iterator(const iterator &rhs): item(item) {}

  __fwdl_iterator<T, Ptr, Ref> operator=(const iterator &rhs) {
    item = rhs.item;
    return *this;
  }
...
namespace std {
  ...
  struct DummyAllocator
  {
	  ...
	  //Commented to avoid statelessness checks on old c++
      //int m_state = 10;
  ...
  template<typename T, typename ALLOCATOR = DummyAllocator<T>>
  class vector {
    T *_start;
    T *_finish;
    T *_end_of_storage;
  ...
  public:
    ...
    typedef __vector_iterator<T, T *, T &> iterator;
    typedef __vector_iterator<T, const T *, const T &> const_iterator;
	typedef __vector_reverse_iterator<iterator> reverse_iterator;
	typedef __vector_reverse_iterator<const_iterator> const_reverse_iterator;
...
namespace std {
  template <class BidirectionalIterator, class Distance>
    static void __advance (BidirectionalIterator& it, Distance n,
                    std::bidirectional_iterator_tag) {
    if (n >= 0) while(n-- > 0) ++it; else while (n++<0) --it;
  }

  template <class RandomAccessIterator, class Distance>
    static void __advance (RandomAccessIterator& it, Distance n,
