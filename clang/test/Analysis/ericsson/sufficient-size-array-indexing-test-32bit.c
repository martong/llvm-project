// RUN: %clang_analyze_cc1 -triple i386 -analyzer-checker=core,alpha.ericsson.SufficientSizeArrayIndexing %s -verify
...
void test_multi_dimensions2() {
  ...
  int x;
  x = array1[i1][i2]; // expected-warning{{Indexing array with type 'unsigned char' cannot cover the whole range of the array's index set, which results in memory waste. Consider using a type with greater maximum value}}
  x = array2[i1][i2]; // expected-warning{{Indexing array with type 'unsigned char' cannot cover the whole range of the array's index set, which results in memory waste. Consider using a type with greater maximum value}}
  x = array3[i1][i2]; // expected-warning{{Indexing array with type 'unsigned char' cannot cover the whole range of the array's index set, which results in memory waste. Consider using a type with greater maximum value}} expected-warning{{Indexing array with type 'unsigned char' cannot cover the whole range of the array's index set, which results in memory waste. Consider using a type with greater maximum value}}
}
